...
form.addEventListener("submit", async (tjek) => {
  //tjek er egentlig en form for objekt der bliver produceret når vi laver den her eventlistener, det er ikke noget vi selv har defineret, men det er det der kommer ind når vi laver en submit event. Det har en masse information om selve eventet, og det er det vi skal bruge til at lave preventDefault, som er det næste vi gør.
...
*/
  ...
  const resolve = await fetch("/api/register", {
    //vi skal fetch noget fra register
  ...
  if (resolve.ok) {
    //Hvis vi får en god respons, så gør vi...
    showSuccess("Registration successful!"); //fortæl brugeren det gik godt
    await delay();
    window.location.href = "/login.html"; //og send dem videre til login siden
  } else {
    showPopup(result.error); //Den her viser til burgerne (alert) en fejl (error) fra (result) som er defineret ovenfor.
